...
java_options() {
    ...
    if [ "x${MODE}" = "xprofiling" ]; then
        ...
        if [ "${OS}" = "Darwin" ]; then
            ...
            JAVA_OPTIONS="${JAVA_OPTIONS} -agentpath:async-profiler/libasyncProfiler_x86_64.so=start,cstack=no,event=cpu,file=${profiling_directory}/%t.jfr"
            echo "Using: x86_64"
        elif [ "${ARCH}" = "aarch64" ]; then
            JAVA_OPTIONS="${JAVA_OPTIONS} -agentpath:async-profiler/libasyncProfiler_aarch64.so=start,cstack=no,event=cpu,file=${profiling_directory}/%t.jfr"
            echo "Using: aarch64"
        else
